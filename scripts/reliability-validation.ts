...
interface Config {
  ...
  SLO_AVAILABILITY_TARGET: number;
}
...
interface AllResults {
  ...
  slo: TestCategory;
  rateLimit: TestCategory;
  circuitBreaker: TestCategory;
  errorRecovery: TestCategory;
...
class TestResults {
  ...
  addResult(category: keyof AllResults, test: string, result: Omit<TestResult, 'timestamp' | 'duration'>): void {
  ...
  getOverallStatus(): { totalTests: number; passedTests: number; failedTests: number; successRate: number; status: 'PASS' | 'FAIL' } {
  ...
  printSummary(): boolean {
    ...
    Object.entries(this.results).forEach(([category, tests]) => {
      const categoryTests = Object.values(tests) as TestResult[];
      ...
      if (categoryTotal > 0) {
        ...
        categoryTests.forEach((test: TestResult) => {
...
async function testMonitoringInfrastructure() {
  ...
  try {
    ...
    testResults.addResult('monitoring', 'prometheus-metrics', {
      ...
      error: metricsCount < 20 ? `Only ${metricsCount} metrics found, expected >= 20` : undefined
    });
...
async function testSLOCompliance() {
  ...
  testResults.addResult('slo', 'api-availability', {
    ...
    error: availability < CONFIG.SLO_AVAILABILITY_TARGET ? 
      `Availability ${(availability * 100).toFixed(3)}% below target ${(CONFIG.SLO_AVAILABILITY_TARGET * 100).toFixed(1)}%` : undefined
  });
  ...
  testResults.addResult('slo', 'response-time', {
    ...
    error: p95Latency > CONFIG.EXPECTED_RESPONSE_TIME_MS ?
      `P95 latency ${Math.round(p95Latency)}ms exceeds target ${CONFIG.EXPECTED_RESPONSE_TIME_MS}ms` : undefined
  });
}
...
async function testErrorRecovery() {
  ...
  try {
    ...
    const isTimeoutError = (error as any).code === 'ECONNABORTED' || (error as any).message?.includes('timeout');
    ...
    testResults.addResult('errorRecovery', 'timeout-handling', {
      ...
      error: !isTimeoutError ? (error as any).message : undefined
    });
  }
  ...
  try {
    ...
    testResults.addResult('errorRecovery', 'database-connection', {
      ...
      metrics: {
        ...
        'response_time_ms': hasDbCheck ? response.data.checks.database.responseTime : undefined
      }
    });
