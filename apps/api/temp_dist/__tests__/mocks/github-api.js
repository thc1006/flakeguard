...
export class MockGitHubApiClient {
    constructor(authToken) {
        ...
        this.requestCount = 0;
        this.maxRequests = 15000;
    }
    async getWorkflowRun(_owner, _repo, runId) {
        this.checkRateLimit();
    ...
    async listWorkflowRunArtifacts(_owner, _repo, runId) {
        this.checkRateLimit();
        this.incrementRequestCount();
        if (!this.authToken) {
            throw this.createError(401, MOCK_UNAUTHORIZED_ERROR);
        ...
        const artifacts = MOCK_ARTIFACTS.filter(artifact => artifact.workflow_run?.id === runId || runId === 987654321);
        return {
            total_count: artifacts.length,
            artifacts
        };
    }
    async downloadArtifact(_owner, _repo, artifactId) {
        this.checkRateLimit();
        this.incrementRequestCount();
        if (!this.authToken) {
            throw this.createError(401, MOCK_UNAUTHORIZED_ERROR);
    ...
    async getArtifact(_owner, _repo, artifactId) {
        this.checkRateLimit();
        this.incrementRequestCount();
        if (!this.authToken) {
            throw this.createError(401, MOCK_UNAUTHORIZED_ERROR);
    ...
    createError(status, body, headers = MOCK_RESPONSE_HEADERS) {
    ...
    generateMockZipContent(artifactName) {
    ...
    getMockXmlContent(artifactName) {
        switch (artifactName) {
            case 'test-results':
...
// ============================================================================
export function createMockGitHubApiClient(authToken = 'mock-token') {
...
export function createMockWorkflowRun(overrides = {}) {
    return {
        ...MOCK_WORKFLOW_RUN,
        ...overrides
    };
...
export function simulateNetworkError(delay = 1000) {
...
export function simulateTimeout(delay = 5000) {
    return new Promise((_, reject) => {
        setTimeout(() => {
...
export function createLargeArtifactResponse(sizeInMB) {
