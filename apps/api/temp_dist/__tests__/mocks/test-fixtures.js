...
export function loadAllFixtures() {
    ...
    for (const [key, path] of Object.entries(FIXTURE_PATHS)) {
        try {
            fixtures[key] = readFileSync(path, 'utf-8');
        }
        catch (error) {
...
// ============================================================================
export const EXPECTED_SUREFIRE_SUCCESS = {
...
export const EXPECTED_SUREFIRE_FAILURES = {
    name: undefined,
    tests: 8,
    failures: 2,
    errors: 1,
...
export const EXPECTED_GRADLE_RESULTS = {
...
export const EXPECTED_JEST_RESULTS = {
    name: 'jest tests',
    tests: 12,
    failures: 2,
    errors: 0,
...
export const EXPECTED_PYTEST_RESULTS = {
    name: undefined,
    tests: 15,
    failures: 3,
    errors: 1,
...
export const EXPECTED_PHPUNIT_RESULTS = {
    name: undefined,
    tests: 14,
    failures: 2,
    errors: 0,
...
export const EXPECTED_LARGE_RESULTS = {
...
export function createTestSuite(options = {}) {
    const { testCount = 5, failureCount = 1, errorCount = 0, skippedCount = 1, withSystemOutput = false, withProperties = false } = options;
    const testCases = [];
    // Create passed tests
...
export function createTestSuites(suiteCount, options = {}) {
    const suites = [];
    let totalTests = 0;
    let totalFailures = 0;
    let totalErrors = 0;
...
// ============================================================================
export function validateTestSuites(testSuites) {
    const errors = [];
...
export function validateTestSuite(suite) {
    ...
    const statusCounts = suite.testCases.reduce((counts, testCase) => {
