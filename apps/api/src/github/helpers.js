...
import { CHECK_RUN_ACTION_CONFIGS, ERROR_MESSAGES, ARTIFACT_TYPES, } from './constants.js';
/**
...
export class GitHubHelpers {
     ...
     */
    async createCheckRun(owner, repo, params, installationId) {
        try {
     ...
     */
    async updateCheckRun(owner, repo, checkRunId, installationId, updates) {
        try {
            const client = await this.authManager.getInstallationClient(installationId);
     ...
     */
    async updateCheckRunWithFlakeActions(owner, repo, checkRunId, installationId) {
        const actions = [
     ...
     */
    async updateCheckRunWithFlakeDetection(owner, repo, checkRunId, installationId, analysis, suggestedActions) {
     ...
     */
    async createFlakeGuardCheckRun(owner, repo, headSha, installationId, params) {
        ...
        if (isFlaky) {
            ...
            summary += suggestedActions.map(action => `- ${CHECK_RUN_ACTION_CONFIGS[action].label}: ${CHECK_RUN_ACTION_CONFIGS[action].description}`).join('\n');
            conclusion = 'neutral';
        }
        else {
     ...
     */
    async createFlakeGuardSummaryCheckRun(owner, repo, headSha, installationId, summary, hasFailures) {
     ...
     */
    async rerunWorkflow(owner, repo, runId, installationId, options = {}) {
        try {
            const client = await this.authManager.getInstallationClient(installationId);
     ...
     */
    async rerunFailedJobs(owner, repo, runId, installationId, options = {}) {
        try {
            const client = await this.authManager.getInstallationClient(installationId);
     ...
     * Cancel workflow run
     */
    async cancelWorkflow(owner, repo, runId, installationId) {
        try {
            const client = await this.authManager.getInstallationClient(installationId);
     ...
     * Get workflow jobs
     */
    async getWorkflowJobs(owner, repo, runId, installationId) {
        try {
            const client = await this.authManager.getInstallationClient(installationId);
     ...
     */
    async listArtifacts(owner, repo, runId, installationId, options = {}) {
        try {
            const client = await this.authManager.getInstallationClient(installationId);
     ...
     */
    async generateArtifactDownloadUrl(owner, repo, artifactId, installationId) {
        try {
            const client = await this.authManager.getInstallationClient(installationId);
     ...
     */
    async createFlakeIssue(owner, repo, installationId, params) {
        try {
            const client = await this.authManager.getInstallationClient(installationId);
    ...
    // UTILITY METHODS
    // =============================================================================
    generateFlakeDetectionSummary(analysis) {
    ...
    inferArtifactType(artifactName) {
 ...
 */
export function createGitHubHelpers(authManager) {
