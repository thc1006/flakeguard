...
import { sanitizeFileName, generateCorrelationId } from './utils.js';
// ============================================================================
// Constants
// ============================================================================
const DEFAULT_ARTIFACT_FILTER = {
...
export class GitHubArtifactsIntegration {
    ...
    ingestionService;
    prisma;
    constructor(authManager, helpers, ingestionService, prisma) {
     ...
     */
    async listWorkflowArtifacts(owner, repo, workflowRunId, installationId, filter = {}) {
        try {
            ...
            const artifacts = await this.fetchAllArtifacts(client, owner, repo, workflowRunId);
            // Apply filters
     ...
     */
    async fetchAllArtifacts(client, owner, repo, workflowRunId) {
        const artifacts = [];
        let page = 1;
        let hasMore = true;
        while (hasMore) {
     ...
     */
    filterArtifacts(artifacts, filter) {
        return artifacts.filter(artifact => {
            ...
            // Check name patterns
            const nameMatches = filter.namePatterns.some(pattern => artifact.name.toLowerCase().includes(pattern.toLowerCase()));
            // Check extensions
            const extensionMatches = filter.extensions.some(ext => artifact.name.toLowerCase().endsWith(ext.toLowerCase()));
     ...
     */
    async generateDownloadUrls(owner, repo, artifactIds, installationId) {
        const downloadInfos = [];
        try {
            const client = await this.authManager.getInstallationClient(installationId);
     ...
     */
    async processWorkflowArtifacts(config) {
        ...
        try {
            ...
            const artifactsResponse = await this.listWorkflowArtifacts(config.repository.owner, config.repository.repo, config.workflowRunId, config.installationId, config.filter);
            ...
            const downloadInfos = await this.generateDownloadUrls(config.repository.owner, config.repository.repo, artifactsResponse.artifacts.map(a => a.id), config.installationId);
            ...
            const artifactSources = downloadInfos.map(info => ({
     ...
     */
    async quickProcessArtifacts(owner, repo, workflowRunId, installationId, filter) {
     ...
     * Store ingestion record in database
     */
    async storeIngestionRecord(config, result) {
        try {
 ...
 */
export function createGitHubArtifactsIntegration(authManager, helpers, ingestionService, prisma) {
    return new GitHubArtifactsIntegration(authManager, helpers, ingestionService, prisma);
}
// ============================================================================
// Utility Functions
// ============================================================================
 ...
 */
export function createTestResultsFilter(overrides = {}) {
    return {
        ...DEFAULT_ARTIFACT_FILTER,
        ...overrides
    };
 ...
 */
export function validateArtifactFilter(filter) {
    const errors = [];
