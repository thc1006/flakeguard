 ...
 */
const SUPPORTED_EXTENSIONS = ['.xml', '.zip', '.tar.gz', '.tgz'];
/**
 * Check if artifact name matches test result patterns
 */
export const isTestResultArtifact = (artifactName) => {
...
export const createArtifactFilter = (options) => {
    const { includePatterns = DEFAULT_TEST_RESULT_PATTERNS, excludePatterns = [], maxSizeBytes = 100 * 1024 * 1024, // 100MB default
    requiredExtensions = SUPPORTED_EXTENSIONS } = options || {};
    return (artifact) => {
...
export const createXMLFileFilter = (options) => {
    const { maxDepth = 10, excludeDirs = ['node_modules', '.git', '__pycache__', 'coverage'], requireTestInPath = false } = options || {};
    return (entry) => {
        ...
        if (excludeDirs.some(dir => name.includes(dir)))
            return false;
        // Check if it's an XML file
        if (!name.toLowerCase().endsWith('.xml'))
            return false;
        // Optionally require 'test' in the path
        if (requireTestInPath && !/test/i.test(name))
            return false;
        return isFile;
    };
};
 ...
 */
export const createJUnitXMLFilter = () => {
    const junitPatterns = [
        /TEST-.*\.xml$/i,
        /.*test.*\.xml$/i,
        /junit.*\.xml$/i,
 ...
 */
export const createChunkBuffer = (targetChunkSize = 64 * 1024) => {
 ...
 */
export const createSizeLimiter = (maxSizeBytes) => {
    let totalSize = 0;
    return new Transform({
        transform(chunk, encoding, callback) {
 ...
 */
export const createTimeoutStream = (timeoutMs) => {
    let timeout;
    let hasData = false;
 ...
 */
export const createRetryableFetch = (config = DEFAULT_RETRY_CONFIG) => {
 ...
 */
export const detectFormatFromPath = (filePath) => {
    const normalized = filePath.toLowerCase();
    for (const [format, indicators] of Object.entries(FORMAT_INDICATORS)) {
        if (format === 'generic')
            continue; // Skip generic for path-based detection
 ...
 */
export const createIngestionError = (type, message, details, fileName, cause) => ({
    type,
    message,
    details,
    fileName,
 ...
 */
export const validateArtifactSource = (artifact) => {
    const errors = [];
